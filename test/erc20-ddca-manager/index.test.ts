...
describe('Test cases for ERC20DDCAManager', function () {
  /**
   *
   * @returns
   */
   ...
   *
   */
  it('Initial token balances on contract is (0,0)', async function () {
  ...
  it('Validate reward distribution between clients', async function () {
    ...
    await createDDCA(ddcaManager, usdt, _quoteTokenAmount, _lotSize2, client1);
    await createDDCA(ddcaManager, usdt, _quoteTokenAmount, _lotSize3, client2);
    ...
    allNodes.forEach((node: any) => {
      ...
      const contribution = lotSize - (lotSize * feePercent) / toBigInt(1000000);
